...
function getMaxScale(scale) {
    ...
    var values = scale.values.filter(function (item) { return !util_1.isNil(item) && !isNaN(item); });
    return Math.max.apply(Math, tslib_1.__spreadArrays(values, [util_1.isNil(scale.max) ? -Infinity : scale.max]));
}
